...
Boolean ProcessFile(char *fn, Network *net, int utterNum, LogDouble currGenBeam, Boolean restartable)
{
    ...
    while(BufferStatus(pbuf)!=PB_CLEARED) {
        ...
        if (trace & T_FRS) {
            ...
            printf("Optimum @%-4d HMM: %s (%s)  %d %5.3f node is %p\n",
                   vri->frame,m->id->name,p, vri->nact,vri->genMaxTok.like/*/vri->frame*/, vri->genMaxNode);
            fflush(stdout);
        }
        nFrames++;
        tact+=vri->nact;
...
void DoAlignment(void)
{
    ...
    while (NumArgs()>0) {
        ...
        if (loadNetworks) {
            ...
            LabList *ll = NULL;
            printf("HVite: lfn is %s, fmt is %d\n", lfn, ifmt);
            trans=LOpen(&netHeap,lfn,ifmt);
            PrintTranscription(trans, lfn);
            if (trans->numLists >= 1) {
