...
/* 该函数的功能是从segStore中找到每个状态n的每个stream对应的观察向量，存储到seqMat[n][s] */
void UCollectData(Sequence **seqMat)
{
  ...
  numSegs = NumSegs(segStore); /* 理解为segStore中存储的segments的个数 */
  for (i=1;i<=numSegs;i++) { /* 遍历每个segment */
    segLen=SegLength(segStore,i); /* segment中观察向量的个数 */
    obsPerState=((float) segLen)/((float) (nStates-2)); /* 平均每个激发状态产生的观察向量的个数 */
    if (obsPerState < 1.0)
      HError(2122,"UCollectData: segment too short[%d]",segLen);
    for (j=1;j<=segLen;j++) {
      obs = GetSegObs(segStore,i,j); /* 从segStore的第i个segment中找到第j个obs */
      n = (int)(((float)(j-1)/obsPerState)+2); /* 计算第j个观察向量对应的激发状态n */
      for (s=1; s<=nStreams; s++){ /* 将obs的观察向量存储到状态n的每个stream中 */
        if (hset.hsKind==DISCRETEHS){
          p = (Ptr)((int)obs.vq[s]);
...
void UniformSegment(void)
{
  ...
  for (n=2; n<nStates; n++) {
    ...
    for (s=1; s<=nStreams; s++){
      size = hset.swidth[s]; /* 一个观察向量的size */
      floor = vFloor[s];
      ste = hmmLink->svec[n].info->pdf+s; /* hmm的第n个状态的第s个stream */
...
/* MakeTraceBack: create the traceBack matrix 创建回溯矩阵 */
...
LogFloat ViterbiAlign(int segNum,int segLen, IntVec states, IntVec *mixes)
{
  ...
  /* 由于只有激发态对观察向量有贡献，所以currState从2开始到nStates-1 */
  for (segIdx=2; segIdx<=segLen; segIdx++) { /* 遍历所有观察向量 */
    ...
    for (currState=2;currState<nStates;currState++) {
      ...
      for (prevState=3;prevState<nStates;prevState++) {
        tranP = hmmLink->transP[prevState][currState]; /* 前一状态转移到当前状态的转移概率 */
        prevP = lastP[prevState]; /* 前一状态为prevState的概率 */
        currP = (tranP<LSMALL) ? LZERO : tranP+prevP; /* 从前一状态prevState跳转到currState状态的概率 */
        if (currP > bestP) { /* 找出最有可能的跳转路径 */
          bestPrevState=prevState; /* 最有可能跳转到当前状态的前一状态 */
          bestP=currP; /* 最有可能转移到currState状态的概率 */
        }
      }
      if (bestP<LSMALL)
        currP = thisP[currState] = LZERO;
      else {
        currP = OutP(&obs,hmmLink,currState); /* 观察向量obs产生currState状态的概率 */
        thisP[currState] = bestP+currP;
      }
      if (trace&T_OBP)
        printf("OutP[s=%d,t=%d] = %f\n",currState,segIdx,currP);
      traceBack[segIdx][currState]=bestPrevState; /* 记录最可能的跳转到当前状态的前一状态，即第segIdx个观察向量最有可能的对应的状态序列 */
    }
  ...
  /* 计算最有可能跳转到最后一个状态nState的bestPrevState和bestP */
  bestPrevState=2;
...
void UpdateCounts(int segNum, int segLen, IntVec states,IntVec *mixes)
{
  ...
  for (i=1; i<=segLen; i++){
    ...
    if (uFlags&(UPMEANS|UPVARS|UPMIXES)){
      ...
      for (s=1; s<=nStreams; s++,ste++){
        ...
        for (j=1; j<=hset.swidth[s]; j++) {
          x = v[j] - mp->mean[j]; /* 更新均值时会加上mp->mean[j]加以抵消，符合htkbook 8.8.1节均值更新公式 */
          ma->mu[j] += x;
          if (uFlags&UPVARS)
            switch(mp->ckind){
            case DIAGC: 
    ...
    if (uFlags&UPTRANS){
      ta->occ[last] += 1.0; /* 从状态i跳转到其他状态的总次数 */
      ta->tran[last][state] += 1.0; /* 记录状态i跳转到状态j的次数 */
      last = state;
...
/* UpTrans: update transition parameters 更新状态转移矩阵，公式参考htkbook 8.8.1小节*/
