...
#define XFORM HTK_XFORM
  ...
  typedef struct _MMFInfo{
    ...
    char *fName;            /* MMF file name [-H]选项输入的hmm文件名*/
    int fidx;               /* MMF file index 按[-H]选项输入的hmm文件个数计数 */
  ...
  typedef struct {        /* 1 of these per stream */
    int nMix;            /* num mixtures in this stream 数据流中高斯混合分量数 */
    MixtureVector spdf;  /* Mixture Vector 高斯混合分量 */
    Ptr hook;            /* general hook */
  }StreamElem;

  typedef struct {       /* HMM状态定义 */
  ...
  typedef struct {
    ...
    int nUse;               /* num logical hmm's sharing this def 共享这个物理hmm的逻辑hmm的数目 */
    Ptr hook;               /* general hook */
  } HMMDef;
  ...
  */
  ...
  typedef struct _MacroDef *MLink; /* hmm宏链表 */

  typedef struct _MacroDef{  /* hmm宏定义 */
    MLink next;             /* next cell in hash table */
    char type;              /* type of macro [hluvixdtmps*] */
    short fidx;             /* idx of MMF file (0 = SMF) hmm文件索引 */
    LabId id;               /* name of macro */
    Ptr structure;          /* -> shared structure or HMM Def 指向共享结构体或定义hmm结构体 */
  } MacroDef;
  ...
  typedef struct _HMMSet{
    ...
    MILink mmfNames;        /* List of external file names 指向[-H]选项指定的hmm文件*/
    ...
    MLink * mtab;           /* Array[0..MACHASHSIZE-1]OF MLink 存储hmm宏的数组，数组充当hash表 */
    ...
    short vecSize;          /* dimension of observation vectors 观察向量的维数 */
