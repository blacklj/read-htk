...
static void StepHMM1(NetNode *node) /* Model internal propagation NBEST */
{
  ...
  /*printf("StepHMM1(), node is %p\n", node);*/
	for (j=2,res=pri->psi->sBuf+2;j<N;j++,res++) {  /* Emitting states first */
		i=seIndex[j][0]; /* begin state which will transition to j */
		endi=seIndex[j][1]; /* end state which j will transition to */
		cur=inst->state+i-1; /* inst->state中存放初始状态信息 */

    /*printf("j = %d i~endi:%d~%d\n", j, i, endi);*/
		...
		for (i++,cur++;i<=endi;i++,cur++) {
			...
			if (res->n==0) { /* find max like token */
    ...
    /*printf("res->tok.like= %f\n", res->tok.like);*/
      ...
      /*printf("state pruning\n");*/
			outp=cPOutP(pri->psi,pri->obs,hmm->svec[j].info,pri->id); /* calculate output probability b_j(O)*/
			res->tok.like+=outp;
...
{	/* 词节点的entry_state和exit_state置空 */
  printf("StepWord1(), init entry and exit state NULL\n");
...
static void StepWord2(NetNode *node) /* Update the path - may be repeated */
{
	...
	inst=node->inst;
  printf("StepWord2(), node is %p, type is %d\n", node, node->type);
	if (node->info.pron==NULL && node->tag==NULL) {
    printf("node->info.pron is null\n");
		...
		inst->exit->tok=inst->state->tok;	/* 将node的entry state的token传递给exit state */
    ...
    printf("inst->exit->tok.path= %p\n", newpth);

		oldpth=inst->state->tok.path;
		if ((newpth->prev=oldpth)!=NULL)	/* 将新的单词的path信息插入到path链表的成员项prev中 */
		  RefPath(oldpth);
...
static void SetEntryState(NetNode *node,TokenSet *src)
{
  ...
  printf("SetEntryState()\n");
	if (node->inst==NULL) {
    AttachInst(node);
    printf("AttachInst a inst for node %d\n", node->type);
  }
	...
	if (res->n==0) {
		if (src->tok.like > res->tok.like) {
		  res->tok=src->tok;	/* node[i]的exit state传递给node[j]的entry state */
      printf("entry state to exit state\n");
    }
	}
	else
...
static void StepInst1(NetNode *node) /* First pass of token propagation (Internal) */
{
  printf("StepInst1(), node is %p\n", node);
	...
	/* Entry tokens valid for t-1, do states 2..N */
	  ...
	  StepWord1(node);
	node->inst->pxd=FALSE;
  printf("not finished for node %d\n", node->type);
}
		...
		/* Must be able to survive doing this twice !! */
{
  ...
  printf("StepInst2(), node is %p type is %d\n", node, node->type);
	if (node_word(node))
	...
	node->inst->pxd=TRUE; /* 仅表示当前inst处理完毕，如果有后续inst，则继续进行stepInst2 */
  printf("current node %d finished\n", node->type);
}
...
static void CreateSEIndex(PSetInfo *psi,HLink hmm)
{
	...
	if (se==NULL) {
		...
		se-=2;
		for (j=2;j<=N;j++) { /*count state transitions boundary for state j according to transitions matrix*/
...
PSetInfo *InitPSetInfo(HMMSet *hset)
{
	...
	psi->sBuf-=1;
	for (i=0; i<psi->max; i++) {
		psi->sBuf[i+1].set=rtoks;rtoks+=MAX_TOKS; /* MAX_TOKS per TokenSet*/
			...
			float scale,LogFloat wordpen,float pscale)
{
    ...
    printf("inst=%p, head=%p, head.link=%p\n", inst, &pri->head, pri->head.link);
	for (inst=pri->head.link;inst!=NULL && inst->node!=NULL;inst=next)
	  if (inst->max<pri->genThresh) {
		  ...
		  pri->nxtInst=inst;
          printf("inst->node type is %d\n", inst->node->type);
		  StepInst2(inst->node);	/* 从起始节点前进到第一个非word节点 */
...
void ProcessObservation(VRecInfo *vri,Observation *obs,int id, AdaptXForm *xform)
{
  ...
  printf("current frame: %d\n", pri->frame);
    ...
    printf("pri->nact is %d > vri->maxBeam is %d\n", pri->nact, vri->maxBeam);
		if (pri->nact>pri->qsn) {
			if (pri->qsn>0)
  ...
  printf("Pass 1...\n");
	pri->genMaxTok=pri->wordMaxTok=null_token;
	pri->genMaxNode=pri->wordMaxNode=NULL;
  printf("initialize pri->genMaxTok and Node NULL\n");
	for (inst=pri->head.link,j=0;inst!=NULL;inst=inst->link,j++) {
    if (inst->node) {
      printf("Pass1: node is %p type is %d\n", inst->node, inst->node->type);
      StepInst1(inst->node);
    }
  }
  ...
  printf("pri->wordThresh= %f, pri->genThresh= %f\n", pri->wordThresh, pri->genThresh);

	/* Pass 2 Performs external token propagation and pruning */
  printf("Pass 2...\n");
	for (inst=pri->head.link,j=0;inst!=NULL && inst->node!=NULL;inst=next,j++) {
    printf("Pass 2: node is %p type is %d\n", inst->node,inst->node->type);
	  if (inst->max<pri->genThresh) {
		  next=inst->link;
      printf("DetachInst node %d\n", inst->node->type);
		  DetachInst(inst->node);
	  }
	  else {
		  pri->nxtInst=inst;
