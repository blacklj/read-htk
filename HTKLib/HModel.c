...
/* EXPORT->AddMMF: Add given file name to set 将[-H]选项指定的hmm文件加入hset->mmfNames链表中*/
MILink AddMMF(HMMSet *hset, char *fname)
{
  MILink p,q;

  HDebug("fname is %s", fname);
  ...
  HDebug("hset->mmfNames is %p", hset->mmfNames);
  if (hset->mmfNames == NULL)
    hset->mmfNames = p;
  else {  /* store in order of arrival 按先后次序组成链表 */
...
MLink NewMacro(HMMSet *hset, short fidx, char type, LabId id, Ptr structure)
{
  ...
  m->next = hset->mtab[hashval]; hset->mtab[hashval] = m; /* hash表项的首部 */
  if (hset->pmap != NULL) {
...
/* LoadAllMacros: loads macros from MMF file fname 从hmm文件中加载hmm宏 */
...
/* CreateHMM: create logical macro. If pId is unknown, create macro for
   it too, along with HMMDef. 创建hmm宏定义，如果hmm不存在，则先创建hmm */
...
/* EXPORT-> OutP: Returns probability (log) of observation x for given state 计算给定状态n产生观察向量x的概率*/
