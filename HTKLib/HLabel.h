...
extern "C" {
  ...
  typedef struct _NameCell{  /* Hash Table Linked List Item hash表项 */
    char * name;             /* Label Name */
    Ptr aux;                 /* User pointer 存放labid的索引 */
  ...
  typedef struct _Label{     /* Information for each label 描述一个label */
  ...
  typedef struct _LabList{   /* 描述一个单词的label */
  ...
  enum _MLFDefType {
    MLF_IMMEDIATE,          /* pattern match */
    MLF_SIMPLE,             /* simple search mode, must include label file name */
    MLF_FULL                /* search labels mode which mirrors a hierarchy of data files */
  };
  ...
  */
  ...
  void PrintList(LabList *ll);
  /*
    Print LabList for debugging
  */
