...
/* create and chain netnode for word inner phones except first and Last */
...
void CreateIEModels(Word thisWord,PronHolder *pInst,int p,int q,
			Network *net,HMMSetCxtInfo *hci)
{	/* create hmm model of node for every phone of a word */
	...
	if (q==p) {
		...
		nfi++; nil++;
		node=NewNode(net->heap,hmm,1);
		node->links[0].node=wordNode;	/* phonetic node followed word end node*/
		node->links[0].like=pInst->fct;
