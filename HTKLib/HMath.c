...
/* EXPORT->LAdd: Return sum x + y on log scale, sum < LSMALL is floored to LZERO 
   log(x+y) = log(e^logx + e^logy) = log(e^x*(1+e^(y-x)) = x + log(1+e^(y-x) */
...
/* EXPORT->LSub: Return diff x - y on log scale, diff < LSMALL is floored to LZERO 
   log(x-y)  = log(e^logx - e^logy) = log(e^x*(1-e^(y-x))) = x + log(1-e^(y-x))*/
